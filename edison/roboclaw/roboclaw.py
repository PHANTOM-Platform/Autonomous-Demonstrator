...
class RoboClaw:
    ...
    def read_encoders(self):
        values = self._read(Cmd.GETENCS, '>II')
        return values[0], values[1]

    def reset_quad_encoders(self):
        self._write(Cmd.RESETENC, '')
    ...
    def read_ispeeds(self):
        return self._read(Cmd.GETISPEEDS, '>ii')

    def drive_speed(self, speed1, speed2):
        self._write(Cmd.MIXEDSPEED, '>ii', speed1, speed2)

    def drive_speed_accel(self, speed1, speed2, accel):
        self._write(Cmd.MIXEDSPEEDACCEL, '>Iii', accel, speed1, speed2)

    def drive_speed_distance(self, speed1, speed2, dist1, dist2, buffer):
        self._write(Cmd.MIXEDSPEEDDIST, '>iIiIB', speed1, dist1, speed2, dist2, buffer)

    def drive_speed_accel_distance(self, speed1, speed2, accel, dist1, dist2, buffer):
        self._write(Cmd.MIXEDSPEEDACCELDIST, '>IiIiIB', accel, speed1, dist1, speed2, dist2, buffer)

    def read_buffer_length(self):
        return self._read(Cmd.GETBUFFERS, '>BB')
