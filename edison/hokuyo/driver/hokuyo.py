...
class Hokuyo(object):
    ...
    def __get_and_parse_scan_without_angles(self, cluster_count, start_step, stop_step):
        distances = []
        result = ''
    ...
    def get_single_scan(self, start_step=START_STEP, stop_step=STOP_STEP, cluster_count=1, type=0):
        self.__port_lock.acquire()
        try:
            ...
            if type == 0:
                scan = self.__get_and_parse_scan_without_angles(cluster_count, start_step, stop_step)
                return scan
            else:
    ...
    def __get_multiple_scans_distances(self, start_step=START_STEP, stop_step=STOP_STEP, cluster_count=1,
                             scan_interval=0, number_of_scans=0):
        self.__port_lock.acquire()
        try:
            cmd = 'MD%04d%04d%02d%01d%02d\n' % (start_step, stop_step, cluster_count, scan_interval, number_of_scans)
    ...
    def __set_scan_distances(self, scan):
        if scan is not None:
            distances = scan

            self.__scan_lock.acquire()
            try:
                self.__angles, self.__distances, self.__timestamp = [], distances, 0
            finally:
                self.__scan_lock.release()

    def get_scan(self):
        if not self.__scanning_allowed:
            scan = self.get_single_scan(type=1)
    ...
    def get_scan_distances(self):
        if not self.__scanning_allowed:
            scan = self.get_single_scan(type=0)
            self.__set_scan_distances(scan)
    ...
    def scanning_distances_loop(self):
        while self.__is_active:
            if self.__scanning_allowed:
                self.__port_lock.acquire()
                for scan in self.__get_multiple_scans_distances():
                    self.__set_scan_distances(scan)
                    if not self.__scanning_allowed or not self.__is_active:
                        self.laser_off()
                        self.laser_on()
