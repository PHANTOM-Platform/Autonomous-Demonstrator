...
static uint64_t SHR3(ts_randomizer_t *d)
...
{	
    ...
    for(;;)
    {
        ...
        d->hz=SHR3(d);		
        d->iz=d->hz&127;		
        if((uint64_t)abs(d->hz)<d->kn[d->iz]) 
...
{ 
    ...
    d->hz = SHR3(d); 
    d->iz = d->hz & 127;
    x= ((uint64_t)abs(d->hz) < d->kn[d->iz])? d->hz * d->wn[d->iz] : ts_random_normal_fix(d); // Generic version
    return x * s + m ;
};
...
int64_t ts_random_int(ts_randomizer_t *d, int64_t min, int64_t max)
{
    ...
    int64_t r;
    r = (int64_t)((max - min + 1) * ts_random(d)) + min; // Multiply interval with random and truncate
    if (r > max) r = max;
...
/*
 * 
 * Function to return most probable position of robot using Monte Carlo algorithm.
 * Parameters:
 *  randomizer  -
 *  scan        -
 *  map         -
 ...
 *  sigma_xy    -
 *  sigma_theta -
 *  stop        - number of MonteCarlo loop iterations
 *  bd          -
 * 
 */ 
ts_position_t ts_monte_carlo_search(ts_randomizer_t *randomizer, ts_scan_t *scan, ts_map_t *map, ts_position_t *start_pos, double sigma_xy, double sigma_theta, int stop, int *bd)
{
    ...
    currentpos = bestpos = lastbestpos = *start_pos;
     ...
     // calculate the distance value for the actual calculated position
    ...
    do 
    {
        ...
        // calculate a new position by randomizing the position variables (starting from the actual position of the robot)
        ...
        // calculate the distance of the new position
        currentdist = ts_distance_scan_to_map(scan, map, &currentpos);
        
        // if the new distance is minor compared to the original distance, save new position as the actual position
