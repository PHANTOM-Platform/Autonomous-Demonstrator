...
/*
 * Function to determine the correct (actual) position of the robot by combining the global map with laser sensor data.
 * 
 * 
 */ 
ts_position_t 
ts_close_loop_position(ts_state_t *state, ts_sensor_data_t *sensor_data, ts_map_t *loop_close_map, ts_position_t *start_position, int *q)
{
    ts_scan_t scan;
    int quality;
    ts_position_t lc_position; // probable location

    // create single scan struct from passed sensor data ('state' used to give laser params)
    ts_build_scan(sensor_data, &scan, state, 1);
    
    // set Monte Carlo loop iteration count = 100000, sigma_xy = 600, sigma_theta = 20
...
void 
ts_close_loop_trajectory(const ts_sensor_data_t *sensor_data, int maxscans, const ts_position_t *startpos, const ts_position_t *close_loop_position)
{
    int i, j;
