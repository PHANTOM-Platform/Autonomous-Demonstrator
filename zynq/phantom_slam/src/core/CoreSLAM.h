...
#define TS_SCAN_SIZE 8192 // maximum number of points stored in a scan
//#define TS_MAP_SIZE 2000 // length in pixels of side of square map (20m x 20m)
#define TS_MAP_SIZE 1200 // length in pixels of side of square map (12m x 12m)  *** TEST ONLY ***
#define TS_MAP_SCALE 0.1  // i.e. 1/10 pixels per mm (or 10 mm/pixel)
#define TS_NO_OBSTACLE 65500 // zero probability value (used for non-existance of obstacle)
#define TS_OBSTACLE 0 // 100% probability value (used for existance of obstacle)
...
typedef struct {
    ...
    int q1, q2;                // Odometry information, left and right wheels.
...
void ts_random_init(ts_randomizer_t *d, uint64_t jsrseed);
double ts_random(ts_randomizer_t *d);
int64_t ts_random_int(ts_randomizer_t *d, int64_t min, int64_t max);
...
void ts_draw_scan(const ts_scan_t *scan, ts_map_t *map, const ts_position_t *pos);
...
void ts_close_loop_trajectory(const ts_sensor_data_t *sensor_data, int maxscans, const ts_position_t *startpos, const ts_position_t *close_loop_position);
#endif // _TINYSLAM_H_
